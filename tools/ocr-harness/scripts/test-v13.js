...
For each PURCHASED item in the transcription, follow these 5 steps (skip promos/coupons/discounts/subtotals/tenders; include "Sales Tax" as a line item):
  ...
  Step 5 (SCORE): score up to 3 categories 0-100.
    ...
    - IMPORTANT: do NOT score a category whose name contains a generic catch-all word ("Supplies", "Goods", "Items", "General", "Miscellaneous") ABOVE a category whose name directly contains the domain word. A directly-named category always wins over a catch-all-named one when both plausibly fit. This rule applies ONLY to catch-all-vs-specific pairings — it does NOT reorder two specific categories whose names both name concrete domains (e.g. Sports vs Books, Pet Care vs Garden — neither contains a catch-all word, so the natural scoring order stands).
