...
fun PieChartEditor(
    ...
    val colors = if (isSystemInDarkTheme()) chartPalette.chartDark else chartPalette.chartLight
    ...
    LaunchedEffect(categories.map { it.id }) {
        ...
        if (!useExisting && totalAmount > 0) {
            ...
            for (i in 0 until n) {
                val cumTarget = com.techadvantage.budgetrak.data.BudgetCalculator.roundCents(totalAmount * (i + 1).toDouble() / n)
                val cumPrev = com.techadvantage.budgetrak.data.BudgetCalculator.roundCents(totalAmount * i.toDouble() / n)
