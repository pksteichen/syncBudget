...
| `checkChatQuota` | v2 Gen 2 callable | SSD §23a.10a | Direct callable (App Check enforced) | `data/ai/HelpChatQuotaGate.kt` → `HelpChatGeminiService.reply` (project-wide daily ceiling on AI calls). Reads/writes `quotaCounters/{feature}` + reads `quotaConfig/{feature}` (server-only via firestore.rules default-deny). Default ceilings: `help_chat`=10000, `ocr`=500, `csv_categorize`=2000. Tunable in Firebase Console without redeploy. Fail-open on internal transaction errors. Client wrapper is also fail-open (4 s timeout, sentinel count=-1) so a function outage doesn't brick the chat. |
...
| `data/ai/HelpChatGeminiService.kt` | ~160 | Single-turn Gemini caller; parses `{ reply, sentiment }`; transient-error retry; wires `usageCallback` to `AnalyticsEvents.logAiCallMetrics`. Calls `HelpChatQuotaGate.check` before generating; on `Decision.Denied` returns `Result.failure(QuotaCeilingReached)`. |
| `data/ai/HelpChatQuotaGate.kt` | ~105 | Kotlin wrapper around the `checkChatQuota` Gen 2 callable. 4 s wire timeout. Sealed `Decision.Allowed/Denied`. Fail-open on timeout / function error / missing response fields. |
...
### 10a.6.5 `HelpChatQuotaGate.kt`

Wraps the server-side `checkChatQuota` Gen 2 callable (see SSD §23a.10a for the architectural rationale and §7.26 of this doc for the Cloud Function row). Called by `HelpChatGeminiService.reply` BEFORE prompt assembly:

```kotlin
sealed class Decision {
    data class Allowed(val count: Int, val ceiling: Int) : Decision()
    data class Denied(val count: Int, val ceiling: Int, val retryAfterSeconds: Long) : Decision()
}
...
Implementation:
- `FirebaseFunctions.getInstance("us-central1")` lazy singleton.
- `withTimeoutOrNull(CALL_TIMEOUT_MS = 4000)` around the callable invocation.
- Any wire-level failure (timeout, function error, App Check failure, missing response data, unparseable JSON) → return `Decision.Allowed(count = -1, ceiling = -1)`. **Fail-open.** Logged via `Log.w` in debug builds only.
- On a `200` response with `allowed = false` → return `Decision.Denied(count, ceiling, retryAfterSeconds)`.
- On a `200` response with `allowed = true` → return `Decision.Allowed(count, ceiling)`.

The `Features` companion object exposes `HELP_CHAT = "help_chat"` constant. Future OCR + CSV gates would add `OCR = "ocr"` + `CSV_CATEGORIZE = "csv_categorize"` and call from those services — the function's `QUOTA_DEFAULT_CEILING` map already accepts those feature names.

**`HelpChatGeminiService.reply` integration:**
...
if (decision is HelpChatQuotaGate.Decision.Denied) {
    return Result.failure(QuotaCeilingReached(count = decision.count, ceiling = decision.ceiling))
}
// proceed to prompt assembly + Gemini call
```

**`QuotaCeilingReached`** is a nested `RuntimeException` subclass inside `HelpChatGeminiService`. Carries `count` and `ceiling` for diagnostic use; the user-visible string (`serviceCeilingReached` in `HelpChatStrings`) is rendered by `HelpChatDialog` after detecting the exception type via `result.exceptionOrNull() is HelpChatGeminiService.QuotaCeilingReached`. Distinct message from the generic `errorReply` so the user understands "tomorrow" not "retry now" is the right next step.

**Daily count is not incremented** on ceiling refusal. `HelpChatStore.incrementDailyCount` is only called on `result.isSuccess` — the user didn't actually consume a turn.
...
| Help Chat cost-ceiling | May 20 2026 | **§10a.6.5 HelpChatQuotaGate + §7.26 Cloud Functions row + Appendix A file count.** New `checkChatQuota` Gen 2 callable (`functions/index.js`) — atomic Firestore transaction on `quotaCounters/{feature}` with daily reset at 00:00 UTC; tunable per-feature ceilings via `quotaConfig/{feature}.dailyCeiling` (defaults: `help_chat`=10000, `ocr`=500, `csv_categorize`=2000). Server-side fail-open on internal transaction error. Client wrapper `data/ai/HelpChatQuotaGate.kt` — 4 s timeout, sealed `Decision.Allowed/Denied`, fail-open on any wire-level failure so a function outage doesn't brick the chat. `HelpChatGeminiService.reply` calls the gate first; on `Decision.Denied` returns `Result.failure(QuotaCeilingReached)` — new sentinel exception. `HelpChatDialog` Send handler distinguishes the new exception via `result.exceptionOrNull() is QuotaCeilingReached` and surfaces the `serviceCeilingReached` localized string instead of the generic `errorReply`. Daily count NOT incremented on ceiling refusal (no turn consumed). Code complete; awaits `firebase deploy --only functions:checkChatQuota`. Companion SSD §23a.10a chapter added. |
