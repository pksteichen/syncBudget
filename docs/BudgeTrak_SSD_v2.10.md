...
Shipped on `dev` 2026-05-20 via the `feature/help-chat` branch merge (commit `59d6601`). Branch deleted post-merge. Cost-ceiling Cloud Function (`checkChatQuota`) added in `71b3581` as a server-side circuit breaker on top of the in-app per-device caps — code ready, awaits `firebase deploy`. See §23a.10a.
...
### 23a.10a Cost-Ceiling Cloud Function (Server-Side Circuit Breaker)

Defense in depth on top of the per-device daily caps. The per-device caps protect ordinary legitimate-user friction but are trustable only if the app is unmodified — they can't see across devices or stop tampered clients, extracted API keys, runaway loops in a future code change, or coordinated emulator-farm abuse. The cost ceiling bounds total project spend regardless of client behavior.

**Function:** `checkChatQuota` (Gen 2 callable, region `us-central1`, `enforceAppCheck = true`). Atomically increments a Firestore counter via a single transaction. Resets daily at 00:00 UTC by comparing the document's `date` field.

**Firestore documents** (server-only — `quotaCounters/*` and `quotaConfig/*` are denied to clients by the default-deny catch-all in `firestore.rules`):

```
quotaCounters/{feature}     // mutable counter
  date: "YYYY-MM-DD" (UTC)
  count: <Int>
  lastUpdated: serverTimestamp

quotaConfig/{feature}       // tunable ceiling (optional — defaults apply if missing)
  dailyCeiling: <Int>
```

**Default ceilings** (per `QUOTA_DEFAULT_CEILING` map in `functions/index.js`):

| feature | default | worst-case $/day (all cache miss) |
|---|---|---|
| `help_chat` | 10,000 | ~$23 |
| `ocr` | 500 (hook for future) | ~$1.50 |
| `csv_categorize` | 2,000 (hook for future) | ~$5 |

Tuning: edit `quotaConfig/{feature}.dailyCeiling` in the Firebase Console (no redeploy required). Set it generous enough that legitimate growth doesn't trip it, but low enough that the worst-case bill is bounded to something you'd notice but not panic over.

**Function response:** `{ allowed: bool, count: Int, ceiling: Int, reason?: string, retryAfterSeconds?: Int }`. Internal transaction errors also return `allowed: true` (server-side fail-open) with `reason: "gate_error_failed_open"` so a function bug doesn't brick chat for users.

**Client integration:**

- `data/ai/HelpChatQuotaGate.kt` — Kotlin wrapper, 4-second wire timeout, fail-open on timeout / function error / missing fields (returns `Decision.Allowed(count = -1, ceiling = -1)`). Sealed `Decision` class so callers can distinguish `Allowed` / `Denied`.
- `HelpChatGeminiService.reply()` calls the gate BEFORE building the prompt. On `Decision.Denied` returns `Result.failure(QuotaCeilingReached(count, ceiling))` — a new sentinel exception nested inside the service.
- `HelpChatDialog` Send handler distinguishes `QuotaCeilingReached` from other failures via `result.exceptionOrNull()` and shows the localized `serviceCeilingReached` string ("Help Chat is taking a short break today…") instead of the generic `errorReply` ("Sorry, I couldn't reach the help service…"). Daily count does NOT increment when the ceiling refuses — the user didn't actually consume a turn.

**Why fail-open?** Cost ceiling is defense-in-depth. The in-app per-device caps still bound legitimate usage even with the function down. Bricking the chat on every function hiccup would be worse for users than the occasional incremental ceiling miss. Cloud Function logs surface ongoing issues; Firebase Console alerts can be configured on `quotaCounters/helpChat.count` if desired.

**Deployment:** `firebase deploy --only functions:checkChatQuota` from a host with Firebase CLI auth. First call seeds the counter document; the config doc is optional (function uses default if missing).
...
| Help Chat cost-ceiling | May 20 2026 | **§23a.10a Cost-Ceiling Cloud Function section added.** Server-side circuit breaker via new `checkChatQuota` Gen 2 callable (App Check enforced; atomic Firestore transaction). Server-only `quotaCounters/{feature}` + `quotaConfig/{feature}` documents (default-deny in `firestore.rules` covers them). Defaults: `help_chat`=10000/day, `ocr`=500/day (future hook), `csv_categorize`=2000/day (future hook). Tunable in Firebase Console without redeploy. Client wrapper (`HelpChatQuotaGate`) is fail-open on every wire-level failure (4 s timeout) so a function outage doesn't brick the chat. New string `serviceCeilingReached` (EN + ES + TC) distinct from the generic `errorReply` — gentle "taking a short break today" tone. Companion LLD §10a.6.5 added. Code complete; awaits `firebase deploy --only functions:checkChatQuota`. |
