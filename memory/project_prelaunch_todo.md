...
1. **App Check → MEETS_DEVICE_INTEGRITY** — Currently `MEETS_BASIC_INTEGRITY`. One-click Firebase Console change. Deferred until live traffic confirms the stricter level doesn't reject legitimate devices.

## Nice to have

2. **Thumbnail loading on IO thread** — Synchronous `BitmapFactory.decodeFile` inside `LazyColumn` composition at `TransactionsScreen.kt:1113` causes scroll jank when many photos are present. Switch to `LaunchedEffect(receiptId) { withContext(Dispatchers.IO) { loadThumb() } }` with a remember-cache.

## Low priority (code quality, no user impact)

3. **Consolidate matching chain** — Duplicate/RE/AE/BudgetIncome matching logic is triplicated across Dashboard quick-add, `TransactionsScreen`, and `WidgetTransactionActivity`. Extract to a single ViewModel callback-based entry point.

4. **Parallel pending receipt uploads** — `ReceiptSyncManager.processPendingUploads` uploads sequentially. Use the same chunked parallel pattern as the download path (5 concurrent) for faster batch photo sync.
