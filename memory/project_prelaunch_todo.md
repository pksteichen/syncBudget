...
3. **Play Billing Layer 2 — server-side purchase verification (anti-piracy)** — assumes Layer 1 client-side IAP from item #13b is already shipped (✅ v2.10.10). Layer 1's 7-day TTL on cached entitlement (`lastSuccessfulBillingCheck` pref + in-memory lock when stale) bounds the buy-then-block-internet-then-refund attack to one billing cycle, but doesn't close it — a determined attacker can re-allow internet just often enough to refresh the timestamp. Local paid-feature flags (`isPaidUser` in SharedPreferences) are also bypassable by anyone who can decompile + re-sign the APK. App Check already stops pirated APKs from using SYNC / cloud receipts / admin features (unrecognized app signature), but local features (CSV/PDF export, receipt capture, unlimited widget transactions, cash-flow simulation) don't require server calls and aren't protected. Fix:
...
4. **Cheap anti-piracy: runtime signature pinning — CODE LANDED 2026-05-11, PIN HASH STILL UNSET.** `BudgeTrakApplication.verifyAppSignature()` hashes the APK's signing certificate(s) at startup and matches against `expectedApkSignatureSha256` (private val). Mismatch → `recordNonFatal` to Crashlytics + `kotlin.system.exitProcess(0)`. Skipped in `BuildConfig.DEBUG`. Multi-signer aware (rotation-safe). Currently `expectedApkSignatureSha256 = ""` (empty) — the check still logs the observed hash to logcat as `AppSignature: observed=...` on every cold start but enforcement is OFF until the constant is populated. **Next step**: capture the SHA-256 from Play Console → App integrity → App signing → "App signing key certificate" → SHA-256 certificate fingerprint (colon-separated hex pairs, e.g. `AB:CD:EF:...`), paste into the constant, commit + ship. Or capture it from a release-build cold-start logcat line on the test device. Catches naive repackaging only; determined attackers strip the check by smali patching — Layer 2 server-side verification (item #3 above) closes the loophole properly.

5. **In-house fallback ad when AdMob can't load** — when `AdView.AdListener.onAdFailedToLoad` fires (network error, no fill, etc.), swap the AdView for a Compose-only `Box` with a static promo image + tap-to-upgrade CTA that launches `vm.launchPaidUpgrade(activity)` or `vm.launchSubscribe(activity)`. Doubles as an anti-piracy disincentive: a free user blocking internet to dodge ads now sees our own ad promoting upgrades. ~50 LOC + a few drawables in `res/drawable/`. Independent of Layer 2; can ship anytime post-launch. Architecturally: hold an `adFailed: Boolean` state in `MainActivity.setContent`, wire `setAdListener` on the `AdView` to flip it on `onAdFailedToLoad` / off on `onAdLoaded`, and conditional-render between `AndroidView(adView)` and our promo `Box` based on the flag.
