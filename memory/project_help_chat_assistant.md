...
description: AI-powered in-app help chat grounded in a comprehensive user-focused knowledge base, with daily caps, off-topic refusal, email escape, sentiment scoring, a 2-day-debounced Play Store review prompt on sentiment ≥ 9, AND a project-wide cost-ceiling Cloud Function (`checkChatQuota`) for server-side defense in depth. Merged into dev as commit 59d6601 (Merge feature/help-chat). The feature/help-chat branch has been deleted. Remaining follow-up before next release dispatch: `firebase deploy --only functions:checkChatQuota` from a host with Firebase CLI auth; on-device manual QA on dev.
type: project
...
- `71b3581` cost-ceiling Cloud Function (`checkChatQuota`) + client gate. Defense in depth on top of the per-device daily caps — bounds total project spend at the Firestore-counter level (10,000 calls/day default for `help_chat`, tunable in Firebase Console via `quotaConfig/helpChat.dailyCeiling`). Gen 2 callable, App Check enforced, atomic transaction. Client (`HelpChatQuotaGate`) is fail-open: timeout / function error / missing fields all return Allowed so a function outage doesn't brick the chat. On Denied → new `QuotaCeilingReached` sentinel exception → dialog shows `serviceCeilingReached` string ("Help Chat is taking a short break today…") distinct from the generic errorReply. Function default-ceiling map also includes future hooks for `ocr` (500/day) and `csv_categorize` (2000/day) — rejects unknown features but doesn't require Firestore config docs to be seeded first. **Not yet deployed**; requires `firebase deploy --only functions:checkChatQuota` from a host with Firebase CLI auth. The new function name `checkChatQuota` is also extensible — to add OCR or CSV gates later, just call `HelpChatQuotaGate.check(Features.OCR)` from those services.
