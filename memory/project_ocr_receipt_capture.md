...
**HTTP path (raw, since 2026-05-18)**: OCR + AI categorization use raw OkHttp calls to `https://generativelanguage.googleapis.com/v1beta/models/{model}:generateContent?key=…` via `data/ocr/GeminiHttpClient.kt`. Replaced the prior `com.google.ai.client.generativeai:0.9.0` SDK because that SDK does not attach `X-Android-Package` / `X-Android-Cert` headers (it has no `Context`), making Google's Android-app key restriction silently no-op. The raw client reads our own signing-cert SHA-1 via `PackageManager.getPackageInfo(GET_SIGNING_CERTIFICATES)` and attaches both headers on every call. Schemas are inlined as `JSONObject` builders in `data/ocr/GeminiSchemas.kt` (replaces the SDK's `Schema.obj`/`Schema.str`). Firebase AI Logic was considered (auto-attaches via Firebase init context) but would require Firebase BOM 33+ — destabilization risk for the existing 32.7.0 stack made raw HTTP the cleaner path. See `reference_gemini_api_key.md` for restriction details.
