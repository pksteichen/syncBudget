...
- Dialog Danger (red) / Warning (orange) — only the `DialogStyle.DEFAULT` header/footer/`DialogPrimaryButton`/`DialogSecondaryButton` follow `surfaceHeader`/`surfaceHeaderText`. DANGER/WARNING headers stay hard-coded red/orange, and `DialogDangerButton`/`DialogWarningButton` stay locked.
...
**Dialog action buttons**: `DialogPrimaryButton` and `DialogSecondaryButton` now follow `surfaceHeader`/`surfaceHeaderText` (the Window Header slots). Secondary container is `lerp(surfaceHeader, surfaceHeaderText, 0.3f)` — 30% toward text — so it reads as a less-saturated sibling of Primary while still in the theme family (was a hardcoded gray pre-2026-05-18). `DialogDangerButton` (red) and `DialogWarningButton` (orange) stay convention-locked. The `ColorsScreen` "New theme"/"New palette" button uses `ScreenPrimaryButton` (Header colors) so it doesn't visually merge with the dialogs it spawns.

**Button border**: every themed button (`DialogPrimary` / `DialogSecondary` / `DialogDanger` / `DialogWarning` / `ScreenPrimary`) draws a 0.5dp `BorderStroke` in its content/text color. Knob lives in the five `BorderStroke(0.5.dp, …)` lines in `Theme.kt` — change all five together for consistency.
...
- **i18n** (2026-05-18): all hardcoded English in `ColorsScreen.kt` migrated to the four-file strings system as `AppStrings.colors` (`ColorsStrings` data class, 36 fields). `EditMode.label()` and `Slot.label()` are now `@Composable` extension functions reading from `LocalStrings`. Don't reintroduce literal English in this file — add new entries to all four files instead.

## ColorWheelPicker state bug (fixed 2026-05-18)
The HSV state vars in `ColorWheelPicker` (`hue` / `sat` / `value` / `hexInput`) used to be keyed on `color` via `remember(color) { … }`. Every emit recreated those state objects, but the `HueSatWheel`'s `pointerInput(Unit)` block captures `onChange` once and holds references to the *original* mutable state — so dragging the wheel after moving the brightness slider snapped value back to the originally-loaded one. Fix: drop the `color` key, seed once from initial color, and let `emit()` keep `hexInput` in sync. Don't reintroduce keyed state here.
