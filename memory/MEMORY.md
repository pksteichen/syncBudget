...
- [Gemini prompt caching — preamble-first ordering](feedback_gemini_prompt_caching.md) — always emit stable preamble (system prompt + KB) FIRST, variable suffix (history + user msg) LAST. No per-device variables (locale, ids) in the preamble — they fragment the implicit cache. Migrate to explicit caching + server proxy at ~10K+ DAU.
